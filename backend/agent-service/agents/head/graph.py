...
#     infer_locked_dish,
#     just_built_from_inventory,
#     resolve_workflow_consults,
# )
# from tools.core.recipe_build import thread_has_kitchen_build_in_thread
...
#     from agents.head.reply_sanitizer import reply_at_confirm_gate, sanitize_head_reply
#
#     text = (reply or "").strip()
#     if not text:
#         return "What would you like to do next?"
#     if reply_at_confirm_gate(text, state):
#         return sanitize_head_reply(text, state=state)
#
#     history = [
...
#         return sanitize_head_reply(text, state=state)
#
#     if "?" in text or reply_at_confirm_gate(text, state):
#         return sanitize_head_reply(text, state=state)
#
...
#     return sanitize_head_reply(f"{text}\n\n{next_q}", state=state)
#
#
...
#             )
#         from agents.head.workflow_engine import ADD_DISH, parse_workflow_state
#         from tools.core.recipe_build import thread_has_recipe_draft
#
#         wf = parse_workflow_state(state.get("workflow_state"))
#         confirm_finalize = (
#             wf is not None
#             and wf.workflow_id == ADD_DISH
#             and wf.step_id == "confirm_finalize"
#             and bool(state.get("confirm_inventory") or state.get("confirm_suggestion"))
#         )
#
#         if (state.get("confirm_inventory") or state.get("confirm_suggestion")) and (
#             recipe_build or thread_has_recipe_draft(thread_history) or confirm_finalize
#         ):
#                 return (
...
#         pick_dish = wf and wf.workflow_id == ADD_DISH and wf.step_id == "pick_dish"
#         unnamed_add = detect_add_dish_build_message(state.get("user_question") or "") and not add_dish and not dish
#
#         if pick_dish or unnamed_add:
#             return (
#                 "\n\n**Your task (Creator Agent):** Chef wants a **new dish** but has not locked a name yet. "
#                 "Propose **2–3 distinct dish ideas** using pantry / expiring ingredients when helpful. "
#                 "For **each** option use `### 1. Dish Name` headers with description, ingredients (qty/unit), "
#                 "numbered prep steps, and a **Visual brief:** line. "
#                 "End with **Suggested Add-ons** (1–3 modifiers with classification and optional price). "
#                 "Do **not** ask which dish to pick or add follow-up questions — Sous Chef closes with one line only. "
...
#                 f"for **{dish}**. "
#                 f"Use `### {dish}` or `### Menu Name: {dish}` as the heading — never prefix with "
#                 f"'Recipe for'. The kitchen menu name must be exactly **{dish}**. "
...
#                 "Close with exactly ONE conversational line ending in "
#                 f"(Yes/No/Update Instructions) — e.g. "
#                 f"Ready to add **{dish or 'the dish'}** to Kitchen with the recipe and suggested add-ons? "
#                 "Do NOT add margin, sell price, or a second question until after Inventory saves."
#             )
#         return (
...
#
#     from agents.head.workflow_engine import advance_workflow_after_turn, parse_workflow_state
#
#     thread = _thread_history_from_state(state)
#     wf = parse_workflow_state(state.get("workflow_state"))
#     kitchen_built = thread_has_kitchen_build_in_thread(thread) or just_built_from_inventory(results)
#     next_wf = advance_workflow_after_turn(
#         wf,
#         consult_results=results,
#         thread_history=thread,
#         kitchen_built=kitchen_built,
#     )
#     if state.get("workflow_state") is not None or wf is not None:
#         update["workflow_state"] = next_wf.to_dict() if next_wf else None
#
#     return update
#
#
