...
def is_valid_recipe_dish_name(name: str) -> bool:
    ...
    if RECIPE_SECTION_HEADERS.match(cleaned):
        return False
    if re.match(r"^full kitchen build\b", cleaned, re.I):
        return False
    return True
...
def clean_menu_dish_name(name: str) -> str:
    ...
    cleaned = re.sub(r"^full kitchen build for\s+", "", cleaned, flags=re.I)
    cleaned = re.sub(r"^full kitchen build\s*[-—:]\s*", "", cleaned, flags=re.I)
