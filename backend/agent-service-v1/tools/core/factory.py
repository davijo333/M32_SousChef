...
from tools.core.catalog_draft_helpers import is_valid_recipe_dish_name, clean_menu_dish_name
...
def make_core_tools_for_agent(
    ...
    *,
    ...
    cues_text: str = "",
    recent_bill_ids: Optional[list[str]] = None,
    ctx: CoreToolContext,
) -> list[Any]:
    rid = restaurant_id
...
def _apply_inventory(restaurant_id: str, user_id: str, ctx: CoreToolContext):
    ...
    def apply_inventory(
        ...
        last_ordered_qty: Optional[float] = None,
    ) -> str:
        ...
        apply_price_change.
        update_ingredient allows only: category, current_qty, reorder_threshold,
        last_purchase_price, last_ordered_qty (never name, brand, or inventory_unit).
        ...
        if act == "update_ingredient":
            ...
            if not ctx.confirm_inventory:
                ...
                return (
                    ...
                    "Allowed updates: category, available qty, reorder level, "
                    "previous cost (last_purchase_price), previous order qty (last_ordered_qty)."
                )
            changes: list[str] = []
            if name.strip() and name.strip().lower() != str(ing.get("name", "")).lower():
                return "Ingredient name cannot be changed from chat — create a new row or use the UI."
            if brand_name.strip():
                return "Brand cannot be changed from chat."
            if inventory_unit.strip():
                return "Inventory unit cannot be changed from chat."
            if category.strip():
            ...
            if last_purchase_price is not None:
                changes.append(f"previous cost → ${last_purchase_price:.2f}")
            if last_ordered_qty is not None:
                changes.append(f"previous order qty → {last_ordered_qty}")
            if not changes:
                return (
                    "Provide fields to update: category, current_qty, reorder_threshold, "
                    "last_purchase_price, last_ordered_qty."
                )
            ...
            ctx.push_pending(
                PendingAction(
                    ...
                    lastPurchasePrice=float(last_purchase_price) if last_purchase_price is not None else None,
                    lastOrderedQty=float(last_ordered_qty) if last_ordered_qty is not None else None,
                )
            )
        ...
        if act == "apply_price_change":
            entity_kind = (slug or name).strip().lower()
            ...
            if addon:
                if sell_price is None or sell_price <= 0:
                    return "Provide sell_price for apply_price_change."
                preview = f"Update add-on **{addon['name']}** sell price to ${sell_price:.2f}."
                if not ctx.confirm_inventory:
                    return preview + "\n\nAsk the chef to confirm before applying."
                ctx.push_pending(
                    PendingAction(
                        kind="update_addon",
                        slug=str(addon.get("slug", slug)),
                        sellPrice=float(sell_price),
                    )
                )
                return preview + "\n\nConfirmed — updating add-on price."
            return f"Menu item not found — provide dish or add-on slug or name (got: {entity_kind or 'empty'})."
...
def _apply_menu(restaurant_id: str, ctx: CoreToolContext):
    ...
    def apply_menu(
        ...
        visual_brief: str = "",
        linked_dish_slugs: Optional[list[str]] = None,
    ) -> str:
        ...
        if act in ("add_suggested_dish", "create_dish"):
            return (
                "Disabled in v1 — use **plan_recipe_build** then **finalize_recipe_build** "
                "for full kitchen builds. Ideas stay in chat until chef confirms build."
            )

        if act in ("plan_recipe_build", "plan_recipe", "recipe_plan"):
            existing = ctx.recipe_build
            if (
                isinstance(existing, dict)
                and existing.get("status") == "ready_to_finalize"
                and existing.get("dishName")
                and existing.get("ingredients")
            ):
                return (
                    format_recipe_build_plan(existing)
                    + "\n\nUsing the confirmed workflow recipe plan — call finalize_recipe_build."
                )
            dish_name = clean_menu_dish_name((name.strip() or "").strip())
            if not dish_name or not is_valid_recipe_dish_name(dish_name):
                ...
                if (
                    ...
                    dish_name = clean_menu_dish_name(draft_name)
            if not dish_name:
        ...
        if act in ("finalize_recipe_build", "commit_recipe_build", "build_recipe"):
            ...
            clean_name = clean_menu_dish_name(str(plan.get("dishName") or ""))
            if clean_name:
                plan = {**plan, "dishName": clean_name}
            ctx.recipe_build = plan
        ...
        if act == "update_dish":
            ...
            if not _catalog_confirmed(ctx):
                ...
                return (
                    f"Current menu item:\n{current}\n\n"
                    "Allowed updates: classification, sell_price, description only."
                )
            changes = []
            if name.strip() and name.strip().lower() != str(dish.get("name", "")).lower():
                return "Dish name cannot be changed from chat."
            ...
            img = image_url.strip() or draft.get("image_url", "")
            if img:
                return "Catalog image updates are not supported in chat attribute updates."
            if not changes:
                return "Provide classification, sell_price, and/or description to update."
        ...
        if act == "update_addon":
            ...
            if not _catalog_confirmed(ctx):
                ...
                return (
                    f"Current add-on:\n{current}\n\n"
                    "Allowed attribute updates: classification, sell_price, description. "
                    "Use link_addon_ingredients or update_addon with linked_dish_slugs for links."
                )
            changes = []
            ...
            if link_only:
                changes.append(f"linked dishes → {len(dish_links or [])}")
            else:
                if name.strip() and name.strip().lower() != str(addon.get("name", "")).lower():
                    return "Add-on name cannot be changed from chat."
                if dish_links is not None:
                    return "Link add-ons to dishes via link_addon_ingredients or link_addons_to_dish workflows."
                ...
                if addon_desc:
                    changes.append("description")
                if image_url.strip() or draft.get("image_url", ""):
                    return "Catalog image updates are not supported in chat attribute updates."
            if not changes:
                return "Provide classification, sell_price, description, or linked_dish_slugs to update."
