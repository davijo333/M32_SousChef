...
/** Appended to dish/add-on image searches — one serving, no brands or text on photo. */
...
function dishImageExtraKeywords(
  ...
  const brief = (overrides?.visualBrief ?? "").trim();
  const description = (overrides?.description ?? dish.description ?? "").trim();
  const parts = [brief, description, MENU_PHOTO_GUIDELINES].filter(Boolean);
  if (parts.length) return parts.join(". ");
  const classification = (overrides?.classification ?? dish.classification ?? dish.category ?? "").trim();
  return classification ? `${classification}. ${MENU_PHOTO_GUIDELINES}` : MENU_PHOTO_GUIDELINES;
}
...
export async function regenerateDishImages(
  ...
  if (mode === "pair") {
    if (existing.length === 1) {
      const fetched = await fetchAgentDishImages({
        ...fetchParams,
    ...
    if (!fetched.length) throw new Error("No suitable dish images found");
    const candidates = await persistCatalogSlotsFromPool("dishes", dish.slug, 2, fetched);
