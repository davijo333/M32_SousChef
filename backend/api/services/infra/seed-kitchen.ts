...
import { dishClassKey, normalizeDishClassification } from "@backend/services/catalog/catalog-classification";
import { buildIngredientSku } from "@backend/services/catalog/ingredient-identity";
import { AddOn } from "@backend/models/AddOn";
import { Dish, type RecipeStatus } from "@backend/models/Dish";
import { Ingredient, type IngredientLabel } from "@backend/models/Ingredient";
import { Recipe } from "@backend/models/Recipe";
import { ensureRecipesForRestaurant } from "@backend/services/recipes/recipe-builder";
import { attachSeedCatalogImages } from "@backend/services/infra/seed-catalog-images";
...
export async function seedKitchenCatalog(restaurantId: string): Promise<SeedKitchenResult> {
  const imageSync = await syncTestCatalogImagesToR2();
  if (imageSync.copied > 0) {
    console.info(`[seed] Restored ${imageSync.copied} catalog images from test/storage/r2`);
  }
  ...
  // Persist optional instruction steps from seed fixtures onto generated recipe rows.
  const recipeInstructionByKey = new Map<string, string[]>();
  for (const dish of dishesDoc.dishes) {
    const steps = (dish.instructions ?? []).map((s) => String(s).trim()).filter(Boolean);
    if (steps.length) recipeInstructionByKey.set(`dish:${dish.slug}`, steps);
  }
  for (const addOn of addOnsDoc.addOns) {
    const steps = (addOn.instructions ?? []).map((s) => String(s).trim()).filter(Boolean);
    if (steps.length) recipeInstructionByKey.set(`addon:${addOn.slug}`, steps);
  }
  for (const [key, steps] of recipeInstructionByKey.entries()) {
    const [kind, targetSlug] = key.split(":");
