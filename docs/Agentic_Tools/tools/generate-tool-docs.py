...
def built_line(built: str, tool_id: str) -> str:
    if built == "Yes":
        return "**Yes** — consolidated `@tool` shipped in `services/agent/tools/core/`."
    if built == "Partial":
...
def render_index(tools: list[dict]) -> str:
    ...
    return (
        ...
        "**Built?**\n"
        "- **Yes** — consolidated `@tool` in `services/agent/tools/core/`\n"
        ...
        "## Code\n\n"
        "- Factory: `services/agent/tools/core/factory.py`\n"
        "- Reads: `services/agent/tools/core/reads.py`\n"
        "- Writes / pending actions: `services/agent/tools/core/writes.py`, `apps/web/src/lib/agent-pending-actions.ts`\n\n"
        "Agent profiles: [docs/Agents/](../Agents/README.md)\n"
    )
