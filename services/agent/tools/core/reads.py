...
def read_business(
    ...
    *,
    ...
    if act in ("top_selling", "top_sellers", "top_dishes"):
        ...
        for order in sales_orders:
            ...
            for item in order.get("items") or []:
                name = str(item.get("name") or item.get("dishName") or "item")
                counts[name] = counts.get(name, 0) + float(item.get("qty", 0) or 0)
        if not counts:
            return "No processed sales in this period."
        ranked = sorted(counts.items(), key=lambda row: row[1], reverse=True)[
            : max(1, min(limit, 20))
        ]
        return "\n".join(f"- {name}: {qty:.0f} sold" for name, qty in ranked)
    ...
    return (
        "Unknown action. Use: finance_summary, top_selling, margins, sales_vs_purchases, "
    ...
    *,
    ...
    if act in ("addons", "addon_catalog"):
        addons = find_many(
            "dishes",
            restaurant_id,
            {"name": 1, "slug": 1, "kind": 1, "classification": 1},
            extra_filter={"kind": "addon"},
            limit=limit,
        )
        if not addons:
