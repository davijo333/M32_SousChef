...
from tools.core.menu_actions import resolve_dish_slug, resolve_ingredient_slug

CatalogKind = Literal["ingredient", "dish"]
...
def format_create_collision(kind: CatalogKind, name: str, lookup: dict[str, Any]) -> str | None:
    ...
    formatter = format_ingredient_summary if kind == "ingredient" else format_dish_summary
    noun = "pantry item" if kind == "ingredient" else "dish"
...
def format_update_miss(kind: CatalogKind, query: str, lookup: dict[str, Any]) -> str | None:
    similar: list[dict[str, Any]] = lookup.get("similar") or []
    if not similar:
        return f"No {kind} matching '{query}'. Use query_{'inventory' if kind == 'ingredient' else 'menu'} search to browse."
    formatter = format_ingredient_summary if kind == "ingredient" else format_dish_summary
    lines = "\n".join(f"- {formatter(row)}" for row in similar[:5])
    return (
